...
def add_noise(qc,noise=0.3,num_process=6):
    ...
    # accumulate_states = pool.starmap(accumulated_state.calc_rho_0,run_arguments)
    # pool.close()
    # pool.join()    
    accumulate_states = accumulated_state.calc_rho_0(iks,jmm1,state,int(j_min),int(j_max),N_in,d_dicke)
