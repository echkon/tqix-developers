...
class Gates(object):
    ...
    def RH(self,theta=None,*args, **kwargs):
        """
        
        :math:`RH(\\H)= e^{-i\\H diag(Jz_de)}`

        :param theta: theta angle
        :type theta: float
        :return: new state after being acted upon by RH
        :rtype: ndarray, tensor, sparse
        """              
        params = {"theta":theta}
        self.check_input_param(params)
        return self.gates(type="RH",*args, **kwargs)
    ...
    def gates(self,type="",*args, **kwargs):
        ...
        for ops in ["x","y","z","+","-"]:
            if ops in type:
                count_ops += 1
        if any([True for gate in ["rn","gms","rp","rh"] if gate in type]):
            count_ops += 2
        if count_ops == 1:
            J = get_J(type)
            if self.use_gpu:
            ...
            if "tat" in type:
                ...
                J0[-2,-2] = self.theta # include t here => theta = t*delta
                J0_spr = csr_matrix(J0)
                if self.use_gpu:
                    expJ = torch.matrix_exp(-1j*J0_spr)
                else:
                    expJ = expm(-1j*J0_spr)        
